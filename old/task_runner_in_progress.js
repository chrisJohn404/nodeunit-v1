 ...
 * This is now reserved for system-level tasks or external binary calls (e.g., uglifyjs).
...
function exec(command, taskName = 'Task') {
    try {
        ...
        if (error.stderr) {
			...
			console.trace() // Debug trace for unknown command
...
function runBrowser() {
    ...
    // Step 2: Minify the file using uglifyjs (External command, remains exec)
    // Requires 'uglify-js' to be installed via npm
    console.log('   -> Minifying browser bundle using uglify-js (via shell)...');
    exec(`node_modules/uglify-js/bin/uglifyjs ${targetFile} > ${path.join(browserDir, 'nodeunit.min.js')}`, 'Browser Build');
