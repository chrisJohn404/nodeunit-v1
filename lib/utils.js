...
exports.httputil = function (cgi, envReady) {
    ...
    var START_PORT = port;
    var END_PORT = START_PORT + 1000;

    function startServer(currentPort) {
        if (currentPort > END_PORT) {
            throw new Error('No available ports found between ' + START_PORT + ' and ' + END_PORT);
        }
        ...
        server.on('error', function(err) {
            if (err.code === 'EADDRINUSE') {
                ...
                startServer(currentPort + 1); // Recurse with a new port
            } else {
                throw err; // Throw any other server-related error
            }
        });

        server.listen(currentPort, hostname, function() {
            // This callback runs ONLY after the server is successfully listening.
            // console.log('... Server is successfully running on port ' + currentPort);
            
            var agent = new http.Agent({ host: hostname, port: currentPort, maxSockets: 1 });
            var client = {
                fetch: function (method, path, headers, respReady) {
                    ...
                    request.on('error', (e) => {
                        console.error(`utils/httputil: Problem with request: ${e.message}`);
                    });
                    request.end();
                }
            };
            
            // Now that everything is ready, call the main test function.
